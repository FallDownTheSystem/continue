import { ChatMessage, CompletionOptions, LLMOptions, RedactedThinkingMessagePart, TextMessagePart, ThinkingMessagePart } from "../../index.js";
...
class Anthropic extends BaseLLM {
  ...
  private convertMessage(message: ChatMessage, addCaching: boolean): any {
    if (message.role === "tool") {
      ...
      if (Array.isArray(message.content)) {
        const thinkingBlocks = message.content.filter(part =>
      ...
      if (Array.isArray(message.content)) {
        const textBlocks = message.content.filter(part => part.type === "text");
    ...
    const convertedContent = (Array.isArray(filteredContent) ? filteredContent : [filteredContent]).map((part, contentIdx) => {
      if (part.type === "text") {
        const newpart = {
          ...part,
          // If multiple text parts, only add cache_control to the last one
          ...(addCaching && contentIdx == (Array.isArray(filteredContent) ? filteredContent.length : 1) - 1
  ...
  protected async *_streamChat(
    ...
    if (options.stream === false) {
      ...
      const thinkingBlocks = data.content.filter((block: any) =>
          ...
          switch (value.delta.type) {
            ...
            case "thinking_delta":
              if (thinkingBlockIndex !== undefined) {
                ...
                const currentContent = thinkingBlocksById.get(thinkingBlockIndex) || "";
            ...
            case "signature_delta":
              if (thinkingBlockIndex !== undefined) {
                // Store the signature
                thinkingSignaturesById.set(thinkingBlockIndex, value.delta.signature);

                // Get the accumulated content
                const thinkingContent = thinkingBlocksById.get(thinkingBlockIndex) || "";
  ...
  private setBetaHeaders(headers: any, shouldCacheSystemMessage: boolean | undefined) {
    const betaValues = new Set<string>();

    // Add from existing header if present
    const existingBeta = headers["anthropic-beta"];
    if (existingBeta && typeof existingBeta === "string") {
      existingBeta.split(",").map(v => v.trim()).forEach(v => betaValues.add(v));
    }
